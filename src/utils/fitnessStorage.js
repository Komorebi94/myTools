...
export function getDayWorkoutRecord (recordList, dateKey) {
    return recordList.find(
        (r) => r.date === dateKey && WORKOUT_STATUSES.includes(r.status)
    ) ?? null
}
...
export function applyDayRollover (state, todayKey = formatDateKey()) {
    ...
    if (next.lastCheckDate === yesterdayKey) {
        const yesterdayStatus = getYesterdayWorkoutStatus(
            next.recordList,
            yesterdayKey
        )
        if (yesterdayStatus !== RECORD_STATUS.FINISH &&
            yesterdayStatus !== RECORD_STATUS.HALF) {
            next.continueDays = 0
            next.weekReward_7 = false
        }
    }
