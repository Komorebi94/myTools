...
export function useFitnessDiscipline () {
    ...
    const todayRecord = computed(() =>
        getDayWorkoutRecord(state.value.recordList, todayKey.value)
    )
    ...
    const canRedeem = computed(() =>
        !isSimMode.value && state.value.totalMoney > 0
    )
    ...
    const syncPushVariantToSession = () => {
        const recommendedId = getRecommendedPushVariant(
    ...
    const liveEarnings = computed(() =>
        calcSimEarnings(state.value.totalCheckDays)
    )
    ...
    const appendRedeemRecord = (record) => {
        state.value.recordList = [...state.value.recordList, record].sort((a, b) =>
    ...
    const completeCheckIn = ({ isExtra = false } = {}) => {
        ...
        const variantBonus = calcVariantUpgradeBonus(
            state.value.lastPushVariant,
            pushIdToday
        )
        if (variantBonus) {
        ...
        setDayWorkoutRecord(buildRecord({
            date: todayKey.value,
