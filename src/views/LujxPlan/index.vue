<template>
	<div class="lujx-plan">
		...
		<header class="app-header">
			...
			<div v-if="activeTab === 'today'" class="week-pill">W{{ currentWeek }}</div>
		</header>

		<div v-if="isMockMode" class="mock-banner" role="status">
			<span>预览 · 第 {{ mockDay }} 天 · 第 {{ currentWeek }} 周（不写入打卡）</span>
			<router-link class="mock-exit" :to="{ path: '/lujx', query: { tab: activeTab } }"
				>退出</router-link
			>
		</div>
		...
		<AppToast
			:message="toast"
			variant="lujx"
			:anchor="showFixedCheckin ? 'checkin' : 'footer'"
		/>
