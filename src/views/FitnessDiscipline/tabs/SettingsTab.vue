<template>
    <div class="settings-tab">
        ...
        <section class="card">
            <h3>科学训练节奏</h3>
            <p class="intro">
                参考常见自重周期化方案（每周加 1–2 次/组，巩固后再升阶，约 4–6 周减量）：
                每 <strong>6 次打卡</strong>为一个微周期，自动切换「积量 → 巩固 → 升阶」。
            </p>
            <ul class="rule-list">
                ...
                <li><strong>放松周</strong>：每满 24 次打卡，量减约 40%，完成仍奖 +{{ REWARDS.DELOAD_COMPLETE }} 元</li>
            </ul>
        </section>
        ...
        <section class="card">
            <h3>奖励随训练跟进</h3>
            <ul class="rule-list">
                <li><strong>每日完成</strong>：{{ REWARDS.DAILY_BASE }} 元起，每完成一个小周期 +1，上限 {{ REWARDS.DAILY_CAP }} 元</li>
                <li><strong>升阶日</strong>：额外 +{{ REWARDS.STEP_DAY_BONUS }} 元</li>
                <li><strong>小周期收官</strong>：额外 +{{ REWARDS.MESO_COMPLETE }} 元（第 6 天完成时）</li>
                <li><strong>动作变式进阶</strong>：首次换更难俯卧撑 +{{ REWARDS.VARIANT_UPGRADE }} 元</li>
                <li><strong>超额完成</strong>：+{{ REWARDS.EXTRA_COMPLETE }} 元 · <strong>每连续 7 天</strong>：+{{ REWARDS.STREAK_7 }} 元（可多次领取）</li>
        ...
        <section class="card danger-card">
            ...
            <button type="button" class="reset-btn" @click="resetStep = 1">
                一键重置所有数据
            </button>
        </section>
