    ...
    <div class="home-tab">
        ...
        <section class="card rules-mini">
            ...
            <ul>
                ...
                <li><span class="tag gain">+{{ REWARDS.EXTRA_COMPLETE }}</span> 超额完成（可选）</li>
                <li><span class="tag gain">+{{ REWARDS.VARIANT_UPGRADE }}</span> 动作变式首次进阶</li>
                <li><span class="tag loss">{{ REWARDS.DAILY_SKIP }}</span> 缺席摆烂</li>
                <li><span class="tag gain">+{{ programMeta.rewardPreview.streakBonus }}</span> 每连续满 7 天</li>
            </ul>
        </section>
            ...
            <div class="stat-item wide">
                ...
                <span class="stat-tags">
                    <template v-if="unlockedBreakthroughs.length">
                        <span
                            v-for="b in unlockedBreakthroughs"
                            :key="b.id"
                            class="mini-tag"
                        >{{ breakthroughShort(b.id) }}</span>
                    </template>
        ...
        <Teleport to="body">
            <Transition name="modal">
                <div v-if="showBreakModal" class="break-modal-root" @click.self="showBreakModal = false">
...
.rhythm-badge {
    ...
    &.accumulate { background: #dbeafe; color: #1d4ed8; }
    &.consolidate { background: #fef3c7; color: #b45309; }
    &.progress { background: #fce7f3; color: #be185d; }
    &.deload { background: #e0e7ff; color: #4338ca; }
}
